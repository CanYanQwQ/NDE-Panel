...
DOCKER_COMPOSEV4_URL="https://raw.githubusercontent.com/CanYanQwQ/NDE-Panel/main/docker-compose-v4.yml"
DOCKER_COMPOSEV6_URL="https://raw.githubusercontent.com/CanYanQwQ/NDE-Panel/main/docker-compose-v6.yml"
GOST_SQL_URL="https://raw.githubusercontent.com/CanYanQwQ/NDE-Panel/main/gost.sql"
# 管理脚本自身的 raw 地址(curl|bash 场景下 tms 命令的兜底下载源)
PANEL_INSTALL_RAW_URL="https://raw.githubusercontent.com/CanYanQwQ/NDE-Panel/main/panel_install.sh"
...
print_access_box() {
  ...
  echo "    项目地址 :  https://github.com/CanYanQwQ/NDE-Panel"
  echo "  ──────────────────────────────────────────────────────"
  echo ""
}
...
is_tms_compose() {
  [ -f docker-compose.yml ] && grep -Eq "teminuosi|canyanqwq|gost-mysql" docker-compose.yml
}
...
purge_panel() {
  ...
  if command -v docker &> /dev/null; then
    ...
    docker rmi -f ghcr.io/canyanqwq/springboot-backend:latest ghcr.io/canyanqwq/vite-frontend:latest ghcr.io/teminuosi/springboot-backend:latest ghcr.io/teminuosi/vite-frontend:latest mysql:5.7 2>/dev/null || true
    # 只清悬空镜像(不动其他应用),回收磁盘
