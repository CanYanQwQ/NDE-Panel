...
// isSingboxRunning 判断 sing-box 是否在运行。
// systemd 节点查服务状态;Alpine/Docker 等无 systemd 节点查 fallback PID。
func isSingboxRunning() bool {
	if !hasSystemd() {
		return singboxProcessRunning()
	}
	out, err := exec.Command("systemctl", "is-active", "sing-box").Output()
	if err != nil {
		// is-active 对非 active 状态会返回非零退出码,这里统一视为未运行
