...
public class InboundServiceImpl extends ServiceImpl<InboundMapper, Inbound> implements InboundService {
    ...
    @Autowired
    private com.admin.mapper.ViteConfigMapper viteConfigMapper;
    @Autowired
    ...
    public R assignUser(InboundUserDto dto) {
        ...
        if (dto.getPublicPort() != null) {
            fdto.setInPort(dto.getPublicPort());
        }
        R fr = createForwardAutoPort(fdto, user, node.getId()); // 端口被占自动上移;显式端口冲突直接失败
    ...
    public R assignAllToUser(InboundUserDto dto) {
        ...
        java.util.List<Long> createdInboundUserIds = new java.util.ArrayList<>();
        java.util.List<Long> createdForwardIds = new java.util.ArrayList<>();
        int publicPortOffset = 0;
        if (dto.getPublicPort() != null && (dto.getPublicPort() < 1 || dto.getPublicPort() > 65535)) {
            return R.err("公网端口必须在1-65535范围内");
        }
        for (Inbound in : inbounds) {
            Integer requestedPort = null;
            if (dto.getPublicPort() != null) {
                long candidate = (long) dto.getPublicPort() + publicPortOffset++;
                if (candidate > 65535) {
                    return R.err("自定义公网端口连续分配后超出65535");
                }
                requestedPort = (int) candidate;
            }
            ...
            R r = assignOneNoPush(in, node, user, userLimiter, dto.getExpTime(), subToken,
                    busyPortsByNode.computeIfAbsent(node.getId(), k -> new java.util.HashSet<>()), requestedPort);
            if (r.getCode() != 0) {
                if (dto.getPublicPort() != null) {
                    rollbackCreatedAssignments(createdInboundUserIds, createdForwardIds);
                    return R.err("自定义公网端口分配失败:" + r.getMsg());
                }
                if (firstError == null) firstError = r.getMsg();
                skipped++;
                continue;
            }
            if (r.getData() instanceof InboundUser) {
                InboundUser created = (InboundUser) r.getData();
                createdInboundUserIds.add(created.getId());
                if (created.getGostForwardId() != null) {
                    createdForwardIds.add(created.getGostForwardId());
                }
            }
    ...
    private void rollbackCreatedAssignments(java.util.List<Long> inboundUserIds, java.util.List<Long> forwardIds) {
        for (Long id : inboundUserIds) {
            if (id != null) {
                inboundUserMapper.deleteById(id);
            }
        }
        for (Long id : forwardIds) {
            if (id != null) {
                try {
                    ...
                    log.warn("回滚自定义端口转发[" + id + "]失败: " + e.getMessage());
                }
            }
        }
    }
    ...
    private R assignOneNoPush(Inbound in, Node node, User user, Integer limiterName, Long expTime, String subToken) {
        return assignOneNoPush(in, node, user, limiterName, expTime, subToken, new java.util.HashSet<>(), null);
    }

    private R assignOneNoPush(Inbound in, Node node, User user, Integer limiterName, Long expTime, String subToken,
                              java.util.Set<Integer> knownBusyPorts) {
        return assignOneNoPush(in, node, user, limiterName, expTime, subToken, knownBusyPorts, null);
    }

    private R assignOneNoPush(Inbound in, Node node, User user, Integer limiterName, Long expTime, String subToken,
                              java.util.Set<Integer> knownBusyPorts, Integer requestedPort) {
        Tunnel tunnel = ensurePortForwardTunnel(node.getId());
        if (tunnel == null) {
            return R.err("创建入站转发隧道失败");
        }
        ...
        R fr = createForwardAutoPort(fdto, user, node.getId(), requestedPort, knownBusyPorts);
        if (fr.getCode() != 0 || fr.getData() == null) {
            return R.err("建转发失败:" + fr.getMsg());
        }
        Forward forward = (Forward) fr.getData();
    ...
    public String buildClashSubscription(String token) {
        ...
        for (InboundUser iu : ius) {
            ...
            String proxyPassword = iu.getPassword();
            if ("shadowsocks".equals(in.getProtocol())) {
                ...
                // Shadowsocks 的密码属于入站配置,不是每个 inbound_user 分配记录的随机密码。
                proxyPassword = cfg.getString("password");
            }
            ...
            java.util.Map<String, Object> proxy = ClashUtil.toProxy(
                    ...
                    iu.getUuid(), proxyPassword, in.getSni(),
        ...
        return ClashUtil.buildConfig(proxies, readClashTemplate());
    }

    /**
     * 读管理员配置的 Clash 订阅模板(vite_config 表,key=clash_template)。
     * 没配过返回 null → ClashUtil 走硬编码默认,不影响原有行为。
     * 模板是完整 YAML,可能很长,靠 SchemaMigration 把 value 列放宽成 MEDIUMTEXT 才存得下。
     */
    private String readClashTemplate() {
        try {
            com.admin.entity.ViteConfig cfg = viteConfigMapper.selectOne(
                    new QueryWrapper<com.admin.entity.ViteConfig>().eq("name", "clash_template").last("limit 1"));
            if (cfg == null || cfg.getValue() == null || cfg.getValue().trim().isEmpty()) {
                return null;
            }
            return cfg.getValue();
        } catch (Exception e) {
            log.warn("读取 Clash 订阅模板失败,使用默认模板: " + e.getMessage());
            return null;
        }
    }
    ...
    private R createForwardAutoPort(ForwardDto fdto, User user, Long nodeId) {
        return createForwardAutoPort(fdto, user, nodeId, null, new java.util.HashSet<>());
    }

    private R createForwardAutoPort(ForwardDto fdto, User user, Long nodeId, Integer requestedPort) {
        return createForwardAutoPort(fdto, user, nodeId, requestedPort, new java.util.HashSet<>());
    }

    /**
     * requestedPort 非空时只尝试用户指定的公网端口,冲突直接返回,绝不静默换端口。
     * 为空时保留原来的自动分配、已知占用缓存和重试逻辑。
     */
    private R createForwardAutoPort(ForwardDto fdto, User user, Long nodeId, Integer requestedPort,
                                    java.util.Set<Integer> knownBusy) {
        if (requestedPort != null) {
            if (requestedPort < 1 || requestedPort > 65535) {
                return R.err("公网端口必须在1-65535范围内");
            }
            fdto.setInPort(requestedPort);
            return forwardService.createForwardForUser(fdto, user.getId().intValue(), user.getUser());
        }
