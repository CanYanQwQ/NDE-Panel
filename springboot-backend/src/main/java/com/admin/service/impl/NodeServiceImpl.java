...
public class NodeServiceImpl extends ServiceImpl<NodeMapper, Node> implements NodeService {
    ...
    public R getAllNodes() {
        ...
        for (Node n : nodeList) {
            Integer inboundCount = inboundMapper.selectCount(
                    new QueryWrapper<com.admin.entity.Inbound>().eq("node_id", n.getId()));
            n.setInboundCount(inboundCount);
            // 没有任何协议时 sing-box 尚未被配置,不应显示为故障。
            n.setSingboxRunning(inboundCount != null && inboundCount > 0
                    ? com.admin.common.utils.WebSocketServer.getSingboxRunning(n.getId())
                    : null);
    ...
    private R buildInstallCommand(Node node) {
        ...
        // 第一部分：下载安装脚本  
        command.append("curl -L https://github.com/CanYanQwQ/NDE-Panel/releases/latest/download/install.sh")
