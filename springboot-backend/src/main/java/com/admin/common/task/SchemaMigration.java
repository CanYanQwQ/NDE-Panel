...
public class SchemaMigration implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        // Clash 订阅模板(clash_template)是完整 YAML,几千字节起步,
        // 而 vite_config.value 建表时是 VARCHAR(200),存不下。整列放宽成 MEDIUMTEXT。
        alterColumnIfShorter("vite_config", "value", "mediumtext",
                "ALTER TABLE `vite_config` MODIFY COLUMN `value` MEDIUMTEXT NULL COMMENT '配置值(Clash 订阅模板等长内容用)'");
    }

    /** 列类型不是期望类型才 ALTER;任何异常都吞掉,不能因为迁移失败导致面板起不来 */
    private void alterColumnIfShorter(String table, String column, String expectType, String ddl) {
        try (Connection conn = dataSource.getConnection()) {
            if (columnExists(conn, table, column)) {
                String type = columnType(conn, table, column);
                if (type != null && type.toLowerCase().contains(expectType)) {
                    return;
                }
            }
            try (Statement st = conn.createStatement()) {
                st.executeUpdate(ddl);
                log.info("表结构迁移: {}.{} 已调整为 {}", table, column, expectType);
            }
        } catch (Exception e) {
            String msg = e.getMessage() == null ? "" : e.getMessage();
            if (msg.contains("Duplicate") || msg.contains("1060")) {
    ...
    private String columnType(Connection conn, String table, String column) throws Exception {
