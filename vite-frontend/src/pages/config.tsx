...
const CONFIG_ITEMS: ConfigItem[] = [
  ...
  {
    key: 'clash_template',
    label: 'Clash / Mihomo 订阅模板',
    description: '自定义 Clash/Mihomo 订阅的完整 YAML。留空使用系统默认模板；proxies 由系统自动注入，proxy_groups 中可用 [{{proxy_names}}] 引用所有节点。模板解析失败时会自动回退默认模板。',
    type: 'textarea',
...
const getInitialConfigs = (): Record<string, string> => {
  ...
  const configKeys = ['app_name', 'captcha_enabled', 'captcha_type', 'ip', 'clash_template'];
      ...
      case 'textarea':
        return (
          <textarea
            ...
            rows={20}
            spellCheck={false}
            className={`w-full rounded-medium border bg-transparent px-3 py-2 font-mono text-xs leading-5 outline-none transition-colors placeholder:text-default-400 focus:border-primary ${
              isChanged ? 'border-warning-300' : 'border-default-200'
            }`}
          />
        );
