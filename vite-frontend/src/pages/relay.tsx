...
export default function RelayPage() {
  ...
  const [assignForm, setAssignForm] = useState<any>({ nodeId: null, nodeName: "", protocolCount: 0, userId: null, speedId: null, expDate: null, flowGb: null, publicPort: null });
  ...
  const openNodeAssign = (n: any, landingId: any, landingName: string, count: number) => {
    setAssignForm({ nodeId: n.id, nodeName: n.name, landingId, landingName, protocolCount: count, userId: null, speedId: null, expDate: null, flowGb: null, publicPort: null });
    setAssignOpen(true);
  };
  ...
  const handleNodeAssign = async () => {
    ...
    try {
      ...
      const node = nodes.find((n) => n.id === assignForm.nodeId);
      if (assignForm.publicPort !== null && assignForm.publicPort !== "" && assignForm.publicPort !== undefined) {
        const publicPort = Number(assignForm.publicPort);
        ...
        <ModalContent>
          ...
          <ModalFooter>
            ...
            <Button
              variant="flat"
              onPress={async () => {
                (await copyTextToClipboard(selfClashUrl))
        ...
        <ModalContent>
          ...
          <ModalBody className="space-y-3">
            ...
            <Input
              ...
              value={assignForm.publicPort ?? ""}
              min={nodes.find((n) => n.id === assignForm.nodeId)?.portSta ?? 1}
              max={nodes.find((n) => n.id === assignForm.nodeId)?.portEnd ?? 65535}
              onChange={(e) => setAssignForm({ ...assignForm, publicPort: e.target.value ? Number(e.target.value) : null })}
              description="单协议使用该端口;批量协议从该端口开始按序分配,已有转发端口不会改变"
            />
          </ModalBody>
          <ModalFooter>
